...
class AuthError extends Error {
  code;
  statusCode;
  constructor(message2, code, statusCode = 401) {
    super(message2);
    this.name = "AuthError";
    this.code = code;
  ...
  return async function verifyToken(token) {
    ...
    if (config.allowedProviders && config.allowedProviders.length > 0) {
      if (!provider || !config.allowedProviders.includes(provider)) {
        throw new AuthError(`Login provider "${provider ?? "unknown"}" is not allowed for this app`, "PROVIDER_NOT_ALLOWED", 403);
      }
    }
    return {
      id: payload.sub,
...
function createSupabaseAuth(config) {
  ...
  if (!config.jwtSecret) {
    console.warn("[supabase-auth-validator] No jwtSecret configured for this app. " + "Falling back to remote JWKS verification (network call + cache). " + "If your Supabase project still uses the legacy HS256 JWT secret, set SUPABASE_JWT_SECRET for faster, offline-capable verification.");
  }
