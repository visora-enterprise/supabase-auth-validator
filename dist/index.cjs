...
function getSignVerifyKey(alg, key, usage) {
  if (key instanceof Uint8Array) {
    ...
    return import_node_crypto4.createSecretKey(key);
  }
  ...
  if (isJWK(key)) {
    if (alg.startsWith("HS")) {
      return import_node_crypto4.createSecretKey(Buffer.from(key.k, "base64url"));
    }
    return key;
  }
...
// node_modules/jose/dist/node/esm/runtime/sign.js
var oneShotSign = import_node_util.promisify(crypto2.sign);
...
// node_modules/jose/dist/node/esm/runtime/verify.js
var oneShotVerify = import_node_util2.promisify(crypto3.verify);
...
var parse = (key) => {
  if (key.d) {
    return import_node_crypto5.createPrivateKey({ format: "jwk", key });
  }
  return import_node_crypto5.createPublicKey({ format: "jwk", key });
};
...
var fetchJwks = async (url, timeout, options) => {
  ...
  const [response] = await Promise.race([import_node_events.once(req, "response"), import_node_events.once(req, "timeout")]);
  if (!response) {
    req.destroy();
